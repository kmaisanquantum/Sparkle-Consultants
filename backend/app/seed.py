...
async def seed_data():
    async with AsyncSessionLocal() as db:
        ...
        # ----------------------------------------------------------------------
        # 1. Immediate, Isolated Seeding / Reconciliation of Administrator User
        # ----------------------------------------------------------------------
        try:
            ...
            if existing_admin:
                logger.info(f"Seed administrator '{admin_email}' exists. Reconciling role and password hash...")
                existing_admin.password_hash = hash_password(admin_password)
                existing_admin.role = "administrator"
                existing_admin.is_active = True
                await db.commit()
            else:
                logger.info(f"Creating seed administrator user '{admin_email}'...")
                ...
                db.add(admin_user)
                await db.commit()
                logger.info("Platform administrator user committed successfully.")
        except Exception as exc:
            logger.error(f"Error seeding platform administrator user: {exc}")
            await db.rollback()
            raise

        # ----------------------------------------------------------------------
        # 2. Seed System Settings & Loan Products (Logical Group)
        # ----------------------------------------------------------------------
        try:
            settings_seed = [
                ("business_name", "Sparkle Consultants", "general", "Trading business name"),
                ("tagline", "A fully online lending service for Papua New Guinea.", "general", "Customer tagline"),
                ("alesco_max_total_deduction_pct", "50.00", "compliance", "Alesco public servant net pay retention ceiling percentage"),
                ("bsp_merchant_id", "SPARKLE_PNG_01", "payments", "BSP online payment merchant identifier"),
            ...
            for key, val, cat, desc in settings_seed:
                ...
                existing_s = (await db.execute(s_stmt)).scalar_one_or_none()
                if not existing_s:
            ...
            prod_ps_stmt = select(LoanProduct).where(LoanProduct.code == "PUBLIC_SERVANT_ALESCO")
            prod_ps = (await db.execute(prod_ps_stmt)).scalar_one_or_none()
            if not prod_ps:
                prod_ps = LoanProduct(
                    id=uuid.uuid4(),
                    tenant_id=tenant_id,
            ...
            await db.commit()
            logger.info("System settings and loan products committed successfully.")
        except Exception as exc:
            logger.warning(f"Warning: Failed to seed system settings / loan products: {exc}")
            await db.rollback()

        # ----------------------------------------------------------------------
        # 3. Optional Sample / Demo Customer & Loan Data
        # ----------------------------------------------------------------------
        try:
            kila_email = "kila.kopi@gov.pg"
            kila_stmt = select(User).where(User.email == kila_email)
            if not (await db.execute(kila_stmt)).scalar_one_or_none():
                prod_ps_stmt = select(LoanProduct).where(LoanProduct.code == "PUBLIC_SERVANT_ALESCO")
                prod_ps = (await db.execute(prod_ps_stmt)).scalar_one_or_none()
                prod_ps_id = prod_ps.id if prod_ps else uuid.uuid4()
            ...
            # Sample Customer 2
            manu_email = "manu.vani@gmail.com"
            manu_stmt = select(User).where(User.email == manu_email)
            if not (await db.execute(manu_stmt)).scalar_one_or_none():
            ...
            # Audit log
            admin_user_stmt = select(User).where(User.email == admin_email)
            admin_user_obj = (await db.execute(admin_user_stmt)).scalar_one_or_none()
            if admin_user_obj:
                db.add(AuditLog(
            ...
            await db.commit()
            logger.info("Sample demo data seeded successfully.")
        except Exception as exc:
            logger.warning(f"Warning: Demo customer/loan sample data seeding failed (non-critical): {exc}")
            await db.rollback()
